...
export const Modal = ({ children, zindex} : { children: React.ReactNode} & {zindex: number}) => {
    return <div 
    className={`absolute h-full overflow-auto bg-[rgba(0,0,0,0.15)] w-full flex justify-center items-center backdrop-blur-sm z-[${zindex}] p-5`}>
        {children}
...
const SearchContent = () => {
    ...
    <div ref = {ref} className="dark:bg-[#1F1F1F]  max-w-[660px] w-full h-full min-h-[50px] max-h-[80vh] rounded-[12px]">
